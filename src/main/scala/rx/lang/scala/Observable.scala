...
trait Observable[+T]
{
  ...
  /**
   * Returns an Observable that merges each item emitted by the source Observable with the value in a
   * Try corresponding to that item that is generated by a selector.
   *
   * @tparam R the type of item emitted by the resulting Observable
   * @param trySelector a functor that returns a Try for when given an item emitted by the source Observable
   * @return an Observable that emits the results of merging the items emitted by the source Observable with
   *         the values in the Trys corresponding to those items, as generated by `trySelector`
   */
  def flatMapTry[R](trySelector: T => Try[R]): Observable[R] = {
    flatMap(t => trySelector(t).toObservable)
  }

  /**
   * Returns an Observable that emits the results of applying a function to the pair of values from the source
   * Observable and a Try corresponding to that item that is generated by a selector.
   *
   * @tparam U the Try's element type
   * @tparam R the type of item emitted by the resulting Observable
   * @param trySelector a function that returns a Try for each item emitted by the source Observable
   * @param resultSelector a function that returns an item based on the item emitted by the source Observable and the
   *                       Try returned for that item by the `trySelector`
   * @return an Observable that emits the items returned by `resultSelector` for each item in the source Observable
   */
  def flatMapTryWith[U, R](trySelector: T => Try[U])(resultSelector: (T, U) => R): Observable[R] = {
    flatMapWith(t => trySelector(t).toObservable)(resultSelector)
  }
