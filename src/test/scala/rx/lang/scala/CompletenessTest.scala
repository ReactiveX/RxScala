...
class CompletenessTest extends JUnitSuite {
  ...
  def correspondenceChanges = Map(
      ...
      "buffer(Long, TimeUnit, Int)" -> "tumblingBuffer(Duration, Int, Scheduler)",
      ...
      "replay(Func1[_ >: Observable[T], _ <: Observable[R]], Long, TimeUnit)" -> "replay(Observable[T] => Observable[R], Duration, Scheduler)",
      ...
      "subscribe()" -> "subscribe()",
      "take(Long, TimeUnit)" -> "take(Duration, Scheduler)",
      ...
      "throttleWithTimeout(Long, TimeUnit)" -> "throttleWithTimeout(Duration, Scheduler)",
